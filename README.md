...
Estimating Planck's constant using LEDs works: one can indeed estimate Planck's contant to within 5-10% using a few LEDs, battery, resistor, and a couple of voltmeters. In one technique,  one generates I-V curves for a variety of LEDS. Another technique uses a capacitor and RC discharge circuit. Both technqiues lead to an emission threshold voltage for a given LED.  
...
A third technique is also done, that involves evaluating LEDs driven at constant current. This technique is bound somewhat more to the real theory of operation for an LED.
